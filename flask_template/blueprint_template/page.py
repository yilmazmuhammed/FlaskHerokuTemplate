...
@blueprint_template_page_bp.route('/example-form')
def example_form_page():
    form = forms.ExampleForm(form_title="Example form")

    if form.validate_on_submit():
        json_data = flask_form_to_dict(request_form=request.form, exclude=[], boolean_fields=[])
        g.args = json_data
        return redirect(url_for("blueprint_template_page_bp.example_form_page"))
    return render_template("blueprint_template/example_page.html", page_info=FormPI(title="Form page title", form=form))
